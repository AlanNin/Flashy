...
export const updateVideo = async (req, res, next) => {
    try {
        ...
        if (request.user.id === video.userId) {
            ...
            return next(createError(403, "¡No puedes actualizar este video!"));
        }
    } catch (error) {
        next(error);
    }
...
export const deleteVideo = async (req, res, next) => {
    try {
        ...
        if (request.user.id === video.userId) {
            ...
            return next(createError(403, "¡No puedes elminar este video!"));
        }
    } catch (error) {
        next(error);
    }
...
export const addView = async (req, res, next) => {
    try {
        ...
        res.status(200).json("La visita ha sido incrementada.")
    } catch (error) {
        next(error);
    }
};
